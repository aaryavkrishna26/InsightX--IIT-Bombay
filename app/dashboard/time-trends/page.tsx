  ...
  const generateData = useCallback(() => {
    setHourly(
      Array.from({ length: 24 }, (_, i) => ({
        ...
        value: randomBetween(i >= 6 && i <= 22 ? 20000 : 5000, i >= 17 && i <= 20 ? 90000 : 60000),
      }))
    );
    setDaily(
      ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((d) => ({
        name: d,
        value: randomBetween(50000, 95000),
      }))
    );
    setMonthly(
      ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"].map((m) => ({
        name: m,
        transactions: randomBetween(400000, 700000),
        revenue: randomBetween(2000000, 5000000),
      }))
    );
    setWeekdayVsWeekend(
      Array.from({ length: 24 }, (_, i) => ({
        name: `${i.toString().padStart(2, "0")}`,
        weekday: randomBetween(5000, 50000),
        weekend: randomBetween(8000, 60000),
      }))
    );
  }, []);
  ...
  return (
    <div className="space-y-6">
        ...
        {peakHour && (
          <div className="flex items-center gap-2 rounded-xl border border-primary/20 bg-primary/5 px-4 py-2">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-primary">{peakHour.name} ({peakHour.value.toLocaleString()} txns)</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        {/* Hourly */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-border bg-card p-6 shadow-sm lg:col-span-2">
            ...
            {monthly.length > 0 && (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={monthly}>
                  ...
                  <Line type="monotone" dataKey="revenue" stroke="oklch(0.72 0.15 195)" strokeWidth={2} dot={{ r: 3 }} name="Revenue" />
                </LineChart>
              </ResponsiveContainer>
            )}
          </div>
        ...
        {/* Weekday vs Weekend */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="rounded-2xl border border-border bg-card p-6 shadow-sm lg:col-span-2">
